"""
Acquaintance / social-graph lookup service.

Self-contained Flask Blueprint: all the UserData / UserDatabase / Api
logic and routes for this one service live in this package. app.py
just imports `acquaintance_bp` and registers it - it doesn't need to
know anything about how this service works internally.

All routes are namespaced so they can't collide with other services:
  - Page:  /acquaintance
  - API:   /api/acquaintance/...
"""
import os
import json
...
# BLUEPRINT SETUP
# --------------------------------------------
# template_folder points at this package's own templates/ dir, so
# render_template() below finds services/acquaintance/templates/acq.html
# without touching the main app's template folder.
acquaintance_bp = Blueprint(
    "acquaintance",
    __name__,
    template_folder="templates",
)

# Path to this service's own data file, relative to this package -
# so it works no matter what directory app.py is launched from.
_SERVICE_DIR = os.path.dirname(os.path.abspath(__file__))
USERS_JSON_PATH = os.path.join(_SERVICE_DIR, "users.json")
...
class Api:
    def __init__(self):
        ...
        # Automatically load this service's own users.json if present
        if os.path.exists(USERS_JSON_PATH):
            print(f"[STARTUP][acquaintance] Loading {USERS_JSON_PATH}...")
            if self.db.load_file(USERS_JSON_PATH):
                print(f"[STARTUP][acquaintance] Loaded {self.db.get_user_count()} users.")
            else:
                print("[STARTUP][acquaintance] Failed to load users.json")
        else:
            print(f"[STARTUP][acquaintance] users.json not found at {USERS_JSON_PATH}. Please add it.")
...
# --------------------------------------------
# SINGLE API INSTANCE FOR THIS SERVICE
# --------------------------------------------
# Created once, when this module is imported by app.py. Holds this
# service's in-memory database - stays isolated from other services.
api = Api()

# --------------------------------------------
# ROUTES
# --------------------------------------------
@acquaintance_bp.route('/acquaintance')
def acquaintance_ui():
    return render_template('acq.html')

# ---- File management ----
@acquaintance_bp.route('/api/acquaintance/get_loaded_files')
def get_loaded_files():
    return jsonify(api.db.get_loaded_files())

@acquaintance_bp.route('/api/acquaintance/get_user_count')
def get_user_count():
    return jsonify(api.db.get_user_count())

# ---- Search ----
@acquaintance_bp.route('/api/acquaintance/search_by_rid')
def search_by_rid():
...
@acquaintance_bp.route('/api/acquaintance/search_by_username')
...
@acquaintance_bp.route('/api/acquaintance/search_with_filters')
...
@acquaintance_bp.route('/api/acquaintance/get_filter_options')
def get_filter_options():
    return jsonify(api.db.get_filter_options())

# ---- User details ----
@acquaintance_bp.route('/api/acquaintance/get_user_details')
...
@acquaintance_bp.route('/api/acquaintance/get_deduced_friends')
...
# ---- Graph ----
@acquaintance_bp.route('/api/acquaintance/get_ego_graph')
def get_ego_graph():
    username = request.args.get('username')
    filename = request.args.get('filename')
...
@acquaintance_bp.route('/api/acquaintance/change_graph_root', methods=['POST'])
...
# ---- Pathfinder ----
@acquaintance_bp.route('/api/acquaintance/find_connection_paths', methods=['POST'])
...
# ---- Advanced (stubbed) ----
@acquaintance_bp.route('/api/acquaintance/get_advanced_results')
def get_advanced_results():
    return jsonify(api.get_advanced_results())

@acquaintance_bp.route('/api/acquaintance/clear_advanced_results', methods=['POST'])
def clear_advanced_results():
    api.clear_advanced_results()
    return jsonify({'status': 'ok'})

@acquaintance_bp.route('/api/acquaintance/run_advanced_scan', methods=['POST'])
...
# ---- Scanner (stubbed) ----
@acquaintance_bp.route('/api/acquaintance/open_browser', methods=['POST'])
def open_browser():
    return jsonify({'message': 'Browser automation is disabled in web version.'})

@acquaintance_bp.route('/api/acquaintance/get_token_from_browser', methods=['POST'])
def get_token():
    return jsonify({'error': 'Token acquisition requires browser automation, not available.'})

@acquaintance_bp.route('/api/acquaintance/start_scan', methods=['POST'])
def start_scan():
    return jsonify({'error': 'Scanner disabled in web version.'})

@acquaintance_bp.route('/api/acquaintance/stop_scan', methods=['POST'])
def stop_scan():
    return jsonify({'message': 'Scanner disabled.'})

@acquaintance_bp.route('/api/acquaintance/get_scan_status')
def scan_status():
    return jsonify('Idle')
